...
module.exports = {
    ...
    searchDMAndDMP: async (req, res) => {
        ...
        try {
            ...
            const whereClause = {
                ...(dmInfoSearch && {
                    username: { startsWith: dmInfoSearch, mode: 'insensitive' },
                }),
            };
    ...
    fetchNearbyUsers: async (req, res) => {
        const { userCoordinates, comparedCoordinates } = req.body;
        const userId = req.user.id;
        console.log(`Query: limit: ${req.query.limit}, page: ${req.query.page}`);
        const limit = parseInt(req.query.limit) || 8;
        const page = parseInt(req.query.page) || 0;
        ...
        try {
            let userLongitude = userCoordinates?.longitude;
            let userLatitude = userCoordinates?.latitude;

            // If userCoordinates is not provided, fetch user's location from the database
            ...
            const totalRows = await prisma.user.count({
                where: {
                    preference: true,
                    longitude: {
                        gte: userLongitude.toString(),
                        lte: comparedCoordinates.longitude.toString(),
                    },
                    latitude: {
                        gte: userLatitude.toString(),
                        lte: comparedCoordinates.latitude.toString(),
                    },
                }
            });

            const getUsersConsideringDistance = await prisma.user.findMany({
                where: {
                    preference: true,
                    longitude: {
                        gte: userLongitude.toString(),
            ...
            res.status(200).json({
                result: getUsersConsideringDistance,
                page: page,
                limit: limit,
                totalRows: totalRows,
                totalPage: totalPage
            ...
            res.status(500).send({
                message: error.message || 'Some error occurred while retrieving users by distance',
            });
        }
    },

    getUsersByCountryOrderedByPodium: async (req, res) => {
        console.log(`Query: limit: ${req.query.limit}, page: ${req.query.page}`);
        const limit = parseInt(req.query.limit) || 8;
        const page = parseInt(req.query.page) || 0;
        ...
        try {
            const podiumCountryUser = await prisma.podium.findFirst({
                where: {
                    category: 'COUNTRY',
                    pays: pays,
                    status: 'isOn',
            ...
            // Fetch users in the same country with pagination
            const usersInSameCountry = await prisma.user.findMany({
                where: {
                    pays: pays,
                    isOnline: true
                },
            ...
            if (podiumCountryUser) {
                const podiumUser = podiumCountryUser.user;
                const index = usersInSameCountry.findIndex(user => user.id === podiumUser.id);
                if (index !== -1) {
                    usersInSameCountry.splice(index, 1);
                }
                usersInSameCountry.unshift(podiumUser);
            }

            // Fetch the total count of users in the same country for pagination purposes
            const totalUsersInSameCountry = await prisma.user.count({
                where: {
                    pays: pays,
                    isOnline: true,
                },
            ...
            const totalPage = Math.ceil(totalUsersInSameCountry / limit);

            res.status(200).json({
                users: usersInSameCountry,
                page: page,
                limit: limit,
                totalUsers: totalUsersInSameCountry,
                totalPage: totalPage,
            });
